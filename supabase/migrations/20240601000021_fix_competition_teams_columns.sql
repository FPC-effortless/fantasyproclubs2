-- Fix competition_teams table by adding all missing columns
-- This migration ensures all necessary columns exist for fantasy functionality
...
BEGIN
    ...
    -- Add position column (already exists but ensuring it's there)
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
...
-- Update existing records to have realistic sample data
DO $$
DECLARE
    ...
    IF v_competition_id IS NOT NULL THEN
        ...
        LOOP
            UPDATE public.competition_teams SET
                "position" = v_position,
                points = GREATEST(0, 90 - (v_position * 5) + (RANDOM() * 10)::INTEGER),
                matches_played = LEAST(38, v_position * 2 + (RANDOM() * 8)::INTEGER),
                wins = GREATEST(0, 25 - v_position + (RANDOM() * 5)::INTEGER),
                draws = (RANDOM() * 8)::INTEGER,
                losses = GREATEST(0, v_position - 1 + (RANDOM() * 5)::INTEGER),
                goals_for = GREATEST(0, 65 - v_position * 2 + (RANDOM() * 15)::INTEGER),
                goals_against = GREATEST(0, 25 + v_position + (RANDOM() * 10)::INTEGER)
...
CREATE INDEX IF NOT EXISTS idx_competition_teams_position 
ON public.competition_teams(competition_id, "position");

CREATE INDEX IF NOT EXISTS idx_competition_teams_points 
ON public.competition_teams(competition_id, points DESC); 